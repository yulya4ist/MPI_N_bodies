...
const int N = 4000;              //количество точек
...
double Red_Black(std::vector<double>& y, std::vector<double>& y_n,
  ...
  if (n_th > 1) {
    ...
    MPI_Request* req_send2 = new MPI_Request[2];
    MPI_Request* req_recv2 = new MPI_Request[2];

    MPI_Request* req_send3 = new MPI_Request[2];
    MPI_Request* req_recv3 = new MPI_Request[2];

    MPI_Request* req_send4 = new MPI_Request[2];
    MPI_Request* req_recv4 = new MPI_Request[2];

    do {
      if (my_id != n_th - 1) {
        //передача вниз
        MPI_Send_init(y_n.data() + el_num[my_id] - 2 * N, N, MPI_DOUBLE,
                      my_id + 1, 5, MPI_COMM_WORLD, req_send1);

        //прием снизу
        MPI_Recv_init(y_n.data() + el_num[my_id] - N, N, MPI_DOUBLE, my_id + 1,
                      6, MPI_COMM_WORLD, req_recv1);

        //передача вниз
        MPI_Send_init(y.data() + el_num[my_id] - 2 * N, N, MPI_DOUBLE,
                      my_id + 1, 5, MPI_COMM_WORLD, req_send2);

        //прием снизу
        MPI_Recv_init(y.data() + el_num[my_id] - N, N, MPI_DOUBLE, my_id + 1, 6,
                      MPI_COMM_WORLD, req_recv2);

        MPI_Send_init(y.data() + el_num[my_id] - 2 * N, N, MPI_DOUBLE,
                      my_id + 1, 5, MPI_COMM_WORLD, req_send3);
        MPI_Recv_init(y.data() + el_num[my_id] - N, N, MPI_DOUBLE, my_id + 1, 6,
                      MPI_COMM_WORLD, req_recv3);

        MPI_Send_init(y_n.data() + el_num[my_id] - 2 * N, N, MPI_DOUBLE,
                      my_id + 1, 5, MPI_COMM_WORLD, req_send4);
        MPI_Recv_init(y_n.data() + el_num[my_id] - N, N, MPI_DOUBLE, my_id + 1,
                      6, MPI_COMM_WORLD, req_recv4);
      } else {
        MPI_Send_init(y_n.data(), 0, MPI_DOUBLE, 0, 5, MPI_COMM_WORLD,
                      req_send1);

        MPI_Recv_init(y_n.data(), 0, MPI_DOUBLE, 0, 6, MPI_COMM_WORLD,
                      req_recv1);

        MPI_Send_init(y.data(), 0, MPI_DOUBLE, 0, 5, MPI_COMM_WORLD, req_send2);

        MPI_Recv_init(y.data(), 0, MPI_DOUBLE, 0, 6, MPI_COMM_WORLD, req_recv2);

        MPI_Send_init(y.data(), 0, MPI_DOUBLE, 0, 5, MPI_COMM_WORLD, req_send3);

        MPI_Recv_init(y.data(), 0, MPI_DOUBLE, 0, 6, MPI_COMM_WORLD, req_recv3);
      ...
      if (my_id != 0) {
        ...
        //передача вверх
        MPI_Send_init(y_n.data() + N, N, MPI_DOUBLE, my_id - 1, 6,
                      MPI_COMM_WORLD, req_send1 + 1);
        ...
        MPI_Send_init(y_n.data() + N, N, MPI_DOUBLE, my_id - 1, 6,
                      MPI_COMM_WORLD, req_send4 + 1);
      } else {
        MPI_Recv_init(y_n.data(), 0, MPI_DOUBLE, n_th - 1, 5, MPI_COMM_WORLD,
                      req_recv1 + 1);

        MPI_Send_init(y_n.data(), 0, MPI_DOUBLE, n_th - 1, 6, MPI_COMM_WORLD,
                      req_send1 + 1);

        MPI_Recv_init(y.data(), 0, MPI_DOUBLE, n_th - 1, 5, MPI_COMM_WORLD,
                      req_recv2 + 1);

        MPI_Send_init(y.data(), 0, MPI_DOUBLE, n_th - 1, 6, MPI_COMM_WORLD,
                      req_send2 + 1);
      ...
      switch (send_type) {
        ...
        case 4:
          if (iterations % 2 == 0) {
            ...
            MPI_Startall(2, req_send2);
            MPI_Startall(2, req_recv2);
          }
          break;
        default:
      }
      ...
      if (send_type == 3 || send_type == 4) {
        ...
        if (send_type == 3) {
          ...
          if (iterations % 2 == 1) {
            ...
            MPI_Waitall(2, req_recv2, MPI_STATUSES_IGNORE);
          }
        }
        //верхняя строка
        int i = 1;
      ...
      if (send_type == 4) {
        if (iterations % 2 == 1) {
          MPI_Startall(2, req_send3);
          MPI_Startall(2, req_recv3);
        } else {
          MPI_Startall(2, req_send4);
          MPI_Startall(2, req_recv4);
        }
      }
      ...
      if (send_type == 3 || send_type == 4) {
        ...
        if (send_type == 3) {
          ...
          if (iterations % 2 == 1) {
            MPI_Waitall(2, req_recv3, MPI_STATUSES_IGNORE);
          } else {
            MPI_Waitall(2, req_recv4, MPI_STATUSES_IGNORE);
          }
        }
...
int main(int argc, char** argv) {
  ...
  if (my_id == 0) {
    ...
    std::cout << std::endl << "Якоби последовательный" << std::endl;
    std::cout << "Время = " << t2 - t1 << std::endl;
    time_seq_J = t2 - t1;
    ...
    t4 = omp_get_wtime();
    time_seq_Z = t4 - t3;
    std::cout << std::endl << "Красно-черные последовательный" << std::endl;
    std::cout << "Время = " << t4 - t3 << std::endl;
    std::cout << "Итерации = " << iterations << std::endl;
    std::cout << "|y - u| = " << norm(y_seq, u_seq, 0, N * N) << std::endl
              << std::endl;
  }
  ...
  for (int send_type = 1; send_type <= 4; ++send_type) {
    if (n_th > 1) {
      ...
      if (my_id == 0) {
        std::cout << "Время = " << t2 - t1 << std::endl;
        std::cout << "Итерации = " << iterations << std::endl;
        std::cout << "Ускорение = " << time_seq_J / (t2 - t1) << std::endl;
      }
      ...
      if (my_id == 0) {
        std::cout << "Время = " << t2 - t1 << std::endl;
        std::cout << "Итерации = " << iterations << std::endl;
        std::cout << "Ускорение = " << time_seq_Z / (t2 - t1) << std::endl;
      }
      y_main(el_num, y, y_gen, displs, n_th, my_id);
      if (my_id == 0)
        std::cout << "|y - u| = " << norm(y_gen, u, 0, N * N) << std::endl
